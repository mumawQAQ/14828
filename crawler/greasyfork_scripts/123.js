...
// @author             F9y4ng
// @description        "elegant Search Engine Jump Assistant" permet aux utilisateurs de passer facilement d'un moteur de recherche spécifique à un autre; prend en charge la suppression de la redirection des liens, le blocage de la publicité, la détection automatique des mises à jour, etc., compatible avec plus d'une douzaine de moteurs de recherche, tels que Baidu, Google, Bing, Duckduckgo, Yandex, vous, etc.
...
~(function (w, tTP) {
  ...
  if (w.trustedTypes?.createPolicy) {
    tTP = w.trustedTypes.createPolicy("default", { createHTML: string => string });
  }
          ...
          ontimeout: () => {
            ...
            GMxmlhttpRequest({
              ...
              onerror: e => {
                if (e.error?.includes("Request was redirected to a not whitelisted URL")) {
                  const rUrl = e.error?.toString().match(/Refused to connect to "([^"]*)"/)[1];
                  if (!rUrl || rUrl?.includes("www.baidu.com/search/error")) {
                    reject(new Error(`URLNotExistError`));
                  }
                  resolve(rUrl);
                } else if (e.responseHeaders?.match(/Location:\s*([\S]+)/)) {
  ...
  !(async function () {
    ...
    function insertButtons() {
      try {
        ...
        userSpan.innerHTML = tTP.createHTML(CONST.buttonCode);
