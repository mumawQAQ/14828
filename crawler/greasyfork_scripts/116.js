// ==UserScript==
// @name         1v1.exe | 1v1.LOL Rapid fire, Instant relead, and more!
// @version      1.2
// @description  1v1.LOL hack script - rapid fire, instant reload.
// @author       Zer0LOL
// @namespace    https://greasyfork.org/en/users/946843-zer0lol-yt
// @match        https://1v1.lol/*
...
wasm.instantiate = async function(bufferSource, importObject) {
    ...
    if(new URLSearchParams( window.location.search ).get('party?=zer0hack') === 'true') {
    ...
    if(new URLSearchParams( window.location.search ).get('party?=zer0hack') === 'true') {
        ...
        localStorage.removeItem('party?=zer0hack');
    }
 
    return result;
};
 
if(new URLSearchParams( window.location.search ).get('party?=zer0hack') === 'true') return;
 
const $moreHacks = document.createElement('a');
const $ads = document.querySelector('ads');
$moreHacks.innerText = 'Click to activate hacks';
$moreHacks.style.display = 'flex';
$moreHacks.style.position = 'absolute';
$moreHacks.style.zIndex = '50';
$moreHacks.style.color = 'red';
$moreHacks.style.backgroundColor = 'black';
$moreHacks.style.cursor = 'pointer';
document.body.prepend($moreHacks);
 
$moreHacks.onclick = function() {
    localStorage.setItem('party?=zer0hack', true);
    location.href = 'https://sites.google.com/view/1v1lolhacks/home';
};
