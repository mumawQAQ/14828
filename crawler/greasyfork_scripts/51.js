...
// @name:zh-CN   	图书互助
// @name:zh-TW    	圖書互助
// @name:en      	图书互助 & Chaoxing Duxiu Book PDF : Ssid Dxid & PDF
// @version			3.7.6
// @exclude			*.jd.com*
// @include			*search*
...
// @description		功能：免注册，支持手机，方便查询全国图书馆参考咨询联盟和各大超星、读秀、龙岩等合作图书馆的试读书籍是否可以付费发起互助，获取全文PDF文件。
// @description:zh-CN   功能：免注册，支持手机，方便查询全国图书馆参考咨询联盟和各大超星、读秀、龙岩等合作图书馆的试读书籍是否可以付费发起互助，获取全文PDF文件。
// @description:zh-tw   功能：免注册，支持手機，方便査詢全國圖書館參攷諮詢聯盟和各大超星、讀秀、龍岩等合作圖書館的試讀書籍是否可以付費發起互助，獲取全文PDF檔案。
// @description:en 图书互助, No registration, Support for mobile devices, check the book numbers (SSID, DXID) of literature libraries such as Super Star, Duxiu, UCDRS, and initiate mutual assistance to obtain the full text.
	...
	if (isSearch&&notAdvsearch&&notBaoku) {
		...
		if ( isDuxiu||isCcelib) {
			...
			for (var i = 0; i < mybooks.length; i++) {
				...
				var scgAnchor="https://u.xueshu86.com/ebook/?book_id="+ssid;
		...
		//深圳文献港，全国图书馆联盟搜索页
			...
			for (var i = 0; i < mybooks.length; i++) {
				...
				var myhref="https://u.xueshu86.com/ebook/?book_id=";
				myhref+=ssid;
	...
	if (myurl.indexOf("bookDetail.jsp?") != -1) {
		...
		if (myurl.indexOf("/views/specific/") == -1) {
			...
			*/
			...
			var oldstr="bookDetail.jsp?dxNumber=";
			var newstr="book.do?go=showmorelib&type=1&dxid=";
			var newAnchor="https://u.xueshu86.com/ebook/?book_id="+ssid;
...
function ssn(){
     let ssn = $('script:contains(send_requestajax)');
     //console.log(ssn);
     let key = /ssn=(\d{3,})/
     return ssn.text().match(key);
}
