...
// 171: fix highlighting in autopaging after turning off; pause highlight renewal when scroll on highlight button; add greasyfork; new highlight removal

//console.time("highlight");
function l(message) {	//debug
...
//{	values >
	...
	var _di = { // en/zh locale string for dialog.
		...
		skip_word_list: ['Skip highlighting of words below:','忽略以下词的高亮：'],
...
// #### Config II #### --------------------------}}
	...
	// Configs
		...
		if (confstr.slice(0,1) === "{") {	//如果设置是对象文本（JSON）
...
// Functions
	...
	function addsheet() {
		...
		sheet = addCSS([
			...
			'#' + PRE + '_words > section  form.' + PRE + '_ctrl{display:inline-block !important;}',
			...
			'#' + PRE + '_words > section td+td+td > input {display:inline;width:1.3em;margin:0.1em 0.1em;background:#FAFAFA;border:none;cursor:pointer;font-size:10pt;color:#4d4d4d;}',
	...
	// build ui
		...
		var td1 = creaElemIn('td', tr_COL);
			td1.setAttribute('style', 'border-right: none; padding: 0 0.3em 0 0; vertical-align: middle;');//width:7.2em;
	...
	function add_word(word) {
		...
		for (var m=0; m<word_tmp_len; m++) {	//遍历新词，逐个添加新词的高亮
	...
	function move(node) {
		if (!node) return;
		if (node.className.indexOf(' wordhighlight_em') == -1) node.className += ' wordhighlight_em';	// 在classname中加上突出样式名
		if (node.getBoundingClientRect) {
			var pos = node.getBoundingClientRect();	// 获取pos元素位置对象（相对窗口显示区域）
			var pos_h = node.offsetHeight;			// 元素高度
			document.documentElement.scrollTop = document.body.scrollTop = pos.top + window.pageYOffset - window.innerHeight/2 + pos_h;	// 滚动位置：元素显示顶边距（不是与页面顶边距），加页面纵位移，减半窗口内高，加元素高度
			if (CFG.show_indc_bar) {	// 如果启用指示条，则创建指示条元素并显示到指定高度
				var _em_bar = creaElemIn('div', document.body);
				var pos_t = getY(node);
				_em_bar.setAttribute('style', 'background:rgba(29,163,63,.3);position:absolute;z-index:99999;width:100%;height:' + pos_h + 'px;top:' + pos_t + 'px; animation-name: fadeit; animation-duration: 3s;');
			}
		} else {
			node.scrollIntoView();
		}
		var move_timer = setTimeout(function(){
			node.className = node.className.replace(' wordhighlight_em','');	// 在classname中去除突出样式名
			if (_em_bar) document.body.removeChild(_em_bar);	// 如果有指示条则删除
		},3000);
	}
	...
	function create_inputlist(words, start) {
		...
		return words.map(function(w, i){
			...
			(!CFG.sort_keywords && positions[_i+1]) || (positions[_i+1] = -1);
	...
	function get_KW_from_URL(sUrl, sArr) {
		...
		if (!!KW) KW = KW.replace(/\+/g,' ');//l(KW);
		if (sArr[0] =='Baidu' && sUrl.indexOf('ie=utf-8') == -1 && sUrl.indexOf('ie=UTF-8') == -1) {
			kwtmp = urlDecode(KW);	// 如果是百度且非utf-8  //2017-5-27百度似乎已经主用utf-8  //20170922 helloBibo 增加了“&& urlsearch.indexOf('ie=UTF-8') == -1”
		} else {
	...
	// 当前高亮动态编号提示：在面板或当前正在查找的高亮关键词按钮上方，显示当前查找到的高亮的动态编号
	function position_box(index) {
		...
		posi_tip.innerHTML = positions[index]+1;	// 内容仅动态编号（index为关键词（按钮）序号，index=0即不指定关键词）
		if (index == 0) {
