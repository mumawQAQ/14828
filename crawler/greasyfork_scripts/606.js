...
// @version      1.1.0
// @description  Google、必应、百度、Yandex、360搜索、谷歌镜像、Fsou侧边栏Chat搜索，即刻体验AI，无需翻墙，无需注册，无需等待！最新体验网址：http://chen666.com/?mod=buy&tid=1
// @author       chen666
...
(function () {
    ...
    const menu_updateChat_id = GM_registerMenuCommand("更新Chat", function (event) {
        GM_openInTab("http://chen666.com/?mod=buy&tid=1")
    }, "updateChat");
    const menu_groupNum_id = GM_registerMenuCommand("交流群", function (event) {
        alert("http://chen666.com/?mod=buy&tid=1")
    }, "groupNum");
    ...
    function setPubkey() {
        ...
        if (GPTMODE && GPTMODE == "CHATGPT") {
            ...
            GM_xmlhttpRequest({
                ...
                onerror: (e) => {
                    localStorage.removeItem("openAIkey")
                    document.getElementById("gptAnswer").innerText = "openAI key获取失败，请点击网址链接获取最新key"
                    return
                }
            });
 ...
 p{white-space: pre-line}
   ...
   <article id="gptAnswer" class="markdown-body"><div id="gptAnswer_inner">版本:1.1.0已启动,部分需要魔法。当前模式: ${localStorage.getItem("GPTMODE") ? localStorage.getItem("GPTMODE") : "默认模式"}<div></article>
    </div><p></p>`
            resolve(divE)
        })
    }
